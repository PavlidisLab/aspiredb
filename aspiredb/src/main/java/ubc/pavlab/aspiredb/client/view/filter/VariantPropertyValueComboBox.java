...
/**
 * Variant Property Value ComboBox
 * 
 * @author frances
 * @version $Id: VariantPropertyValueComboBox.java,v 1.7 2013/06/28 18:06:00 frances Exp $
 */
public class VariantPropertyValueComboBox extends SuggestionComboBox<Serializable> {
	private static final int MIN_CHARS = 1;

	private final VariantServiceAsync variantService = GWT.create( VariantService.class );

	public VariantPropertyValueComboBox(final Property property, boolean shouldValidateOnBlur) {
		super(
		...
		this.setDataProxy(
			new RpcProxy<PagingLoadConfig, PagingLoadResult<Serializable>>() {
		        ...
		        public void load(PagingLoadConfig loadConfig, final AsyncCallback<PagingLoadResult<Serializable>> callback) {
		    		...
		    		variantService.suggestValues(property, suggestionContext, new AsyncCallback<Collection<PropertyValue>>() {
		                @Override
