 ...
 * 
 * CRUD-Proxy for DWR 3.0. Coded based on the idea of http://kalmatrongeorgia.ge/dwrExtjs4Store/DwrProxy.js and the
 * @class Ext.data.proxy.Server, Ext.data.proxy.AjaxProxy,Ext.data.proxy.RestProxy
 * 
 */
Ext.define('Ext.ux.DwrProxy', {
	...
	/**
	 * @cfg {Boolean} passDwrStoreParams Send store parameter like "limit","start","page" to Dwr
	 */
	...
	constructor : function(config) {
		...
		/**
		 * @cfg {Object} extraParams Extra parameters that will be included on every request. Individual requests with
	...
	doRequest : function(operation, callback, scope) {
		...
		/*
		 * var loadParams = operation.dwrParams || me.dwrParams; if (typeof (loadParams) === 'object') { dwrParams =
		 * dwrParams.concat(loadParams); }
		 */
		...
		if (Ext.isFunction(me.dwrFunction)) {
			...
			switch (operation.action) {
				...
				case 'destroy' :
					...
					break;
				default :
					me.processResponse(false, operation, request, 'operation "' + operation.action + '" not available',
							callback, scope);
			}
		}
	 ...
	 *            operation The {@link Ext.data.Operation Operation} object to execute
	...
	buildRequest : function(operation) {
		 ...
		 * ServerProxy and any of its subclasses we add both request and response as they may be useful for further
		 * processing
		 */
	...
	/**
	 * @private Copy any sorters, filters etc into the params so they can be sent over the wire
	 */
	getParams : function(operation) {
		var me = this, params = {}, isDef = Ext.isDefined, groupers = operation.groupers, sorters = operation.sorters, filters = operation.filters;
	 ...
	 * 
	 * @param {Ext.util.Sorter[]}
	 *            sorters The array of {@link Ext.util.Sorter Sorter} objects
	 ...
	 * 
	 * @param {Ext.util.Filter[]}
	 *            filters The array of {@link Ext.util.Filter Filter} objects
