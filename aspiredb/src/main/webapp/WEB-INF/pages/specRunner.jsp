<html>
<head>
<title>Test Subject Application</title>

<!-- including the Jasmine Files -->
<link rel="stylesheet" type="text/css"
	href="scripts/ASPIREdb/test/jasmine/jasmine.css">
<script type="text/javascript"
	src="scripts/ASPIREdb/test/jasmine/jasmine.js"></script>
<script type="text/javascript"
	src="scripts/ASPIREdb/test/jasmine/jasmine-html.js"></script>

<!-- including the ExtJS Files -->
<!-- <script type="text/javascript" src="extjs-4.1.1/ext-all.js"></script> -->
<!--  <link rel="stylesheet" type="text/css" href="extjs-4.1.1/resources/css/ext-all.css"> -->

<script src="scripts/lib/ext-all-debug-w-comments.js"></script>

<script type="text/javascript" src="scripts/lib/ext-theme-steelblue.js"></script>

<link rel="stylesheet" type="text/css"
	href="scripts/lib/resources/css/ext-all-steelblue.css">



<!-- including the Jasmine Test Case file -->
<script type="text/javascript"
	src="scripts/ASPIREdb/test/spec/LoginControllerSpec.js"></script>

<!-- including the logintest-app.js file to load test cases -->
<script type="text/javascript"
	src="scripts/ASPIREdb/test/test/logintest-app.js"></script>



</head>
<body>
<%@include file="/common/analytics.jsp" %>
</body>